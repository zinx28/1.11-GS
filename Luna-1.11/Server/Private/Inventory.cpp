...
// ngl best function
int Inventory::GetMaxAmmo(UFortWeaponItemDefinition* ItemDef) {
...
void Inventory::Update(AFortPlayerControllerAthena* PC, FFortItemEntry* Entry) {
...
int Inventory::GetOpenSlot(AFortPlayerControllerAthena* PC) {
	auto Quickbars = PC->QuickBars;
...
int Inventory::GetWeaponSlot(AFortPlayerControllerAthena* PC, FGuid Item) {
	auto Quickbars = PC->QuickBars;
	if (Quickbars != nullptr) {
		for (int i = 0; i < Quickbars->PrimaryQuickBar.Slots.Num(); i++) {
			if (Quickbars->PrimaryQuickBar.Slots[i].Items.Num() > 0) {
...
UFortWorldItem* Inventory::GetItemInInv(AFortPlayerControllerAthena* PC, UFortItemDefinition* Def) {
...
FFortItemEntry Inventory::GetEntryInInv(AFortPlayerControllerAthena* PC, FGuid GUID) {
...
FGuid Inventory::GetItemInSlot(AFortPlayerControllerAthena* PC, int Slot, EFortQuickBars QB) {
	auto Quickbars = PC->QuickBars;
	if (Quickbars)
...
std::pair<EFortQuickBars, int> Inventory::GetItemSlot(AFortPlayerControllerAthena* PC, FGuid GUID) {
	auto Quickbars = PC->QuickBars;
	if (Quickbars) {
		for (int i = 0; i < Quickbars->PrimaryQuickBar.Slots.Num(); i++) {
			// yes this is stupid
...
void Inventory::SpawnPickup(UFortItemDefinition* Item, FVector Loc, int Count, int32 LoadedAmmo) {
	FRotator Rotator = {};
...
void Inventory::RemoveItem(AFortPlayerControllerAthena* PC, FGuid GUID, bool ShouldSpawnItem, int Amount, int ShouldINotSwap) {
...
bool Inventory::IsPrimaryQuickBar(UFortItemDefinition* Def)
{
	return GetQuickBars(Def) == EFortQuickBars::Primary && Def->IsA(UFortWeaponMeleeItemDefinition::StaticClass());
}

int32 Inventory::GetPrimaryQuickBarItems(AFortPlayerControllerAthena* PC)
{
	int Items = 0;
...
UFortWorldItem* Inventory::GiveItem(AFortPlayerControllerAthena* PC, UFortItemDefinition* ItemDef, int Count, int Slot, int LoadedAmmo, bool AutoEquip) {
...
void Inventory::UpdateLoadedAmmo(AFortPlayerControllerAthena* PC, AFortWeapon* Weapon/*, int Ammo*/) {
	auto WorldInventory = PC->WorldInventory;
