...
inline void (*CallPreReplication)(AActor* Actor, UNetDriver* NetDriver) = decltype(CallPreReplication)(ImageBase + 0x1761700);
...
// so lets talk abnout no one closes the channel?!?!?!
// sub_7FF76841E750(__int64 a1, __int64 a2, __int64 a3, __int64 a4) ~ 418/8 is index
static bool (*IsNetRelevantFor)(AActor*, AActor*, AActor*, FVector&) = decltype(IsNetRelevantFor)(ImageBase + 0x1775200);
static void (*CloseActorChannel)(UActorChannel*) = decltype(CloseActorChannel)(ImageBase + 0x1953080);
...
inline FNetworkObjectList& GetNetworkObjectList(UNetDriver* Driver) {
...
namespace Driver {
	...
	inline AActor* GetArcheType(AActor* Actor) {
		// most 1:1 function
	...
	inline int32 ServerReplicateActors_PrepConnections(UNetDriver* NetDriver, float a2) {
	...
	inline std::vector<FNetworkObjectInfo*> ServerReplicateActors_BuildConsiderList(UNetDriver* Driver, float a2) {
	...
	inline UActorChannel* FindActorChannel(UNetConnection* Connection, AActor* Actor) {
	...
	inline AActor* GetNetOwner(AActor* Actor) {
		if (Actor) {
	...
	inline UNetConnection* IsActorOwnedByAndRelevantToConnection(AActor* Actor, const TArray<FNetViewer>& ConnectionViewers, bool& bOutHasNullViewTarget)
	{
	...
	inline bool IsActorRelevantToConnection(AActor* Actor, TArray<FNetViewer>& ConnectionViewers) {
	...
	inline bool IsLevelInitalizedForActor(UNetDriver* Driver, const AActor* InActor, const UNetConnection* InConnection)
	{
	...
	inline int32 ServerReplicateActors_ProcessActors(UNetDriver* NetDriver, UNetConnection* Connection, TArray<FNetViewer>& ConnectionViewers, std::vector<FNetworkObjectInfo*> Actors, int32 FinalSortedCount, int32& OutUpdated) {
	...
	inline bool ShouldActorGoDormant(AActor* Actor, const TArray<FNetViewer>& ConnectionViewers, UActorChannel* Channel, const float Time, const bool bLowNetBandwidth) {
	...
	inline int32 ServerReplicateActors_PrioritizeActors(UNetDriver* Driver, UNetConnection* Connection, TArray<FNetViewer>& ConnectionViewers, std::vector<FNetworkObjectInfo*> ConsiderList, const bool bCPUStatured, std::vector<FNetworkObjectInfo*>& OutPriorityList)
	{
		Driver->GetNetTag()++;
	...
	inline int32 ServerReplicateActors(UNetDriver* Driver, float a2) {
	...
	inline void (*TickFlush_OG)(UNetDriver* Driver, float a2);
